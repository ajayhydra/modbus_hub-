...
// Modbus protocol exception (FC | 0x80 response). Returns the last exception
// code (1..11) seen on this context after a -1 return from a read/write call,
// or 0 if the failure was a transport / timeout error rather than a protocol
// exception. Cleared at the start of every read/write operation.
int modbus_get_last_exception(modbus_t* ctx);

// Human-readable name for a Modbus exception code (e.g. 2 -> "Illegal data
// address"). Returns "Unknown exception" for codes outside 1..11.
